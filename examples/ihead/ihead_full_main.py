...
if __name__ == '__main__':
    ...
    run = wandb.init(
        ...
        project = "Bayeformers",
        name = "Full unfreezed transformer",
    )
    ...
    for i, (x, y, outs) in enumerate(iterate_batches(ds, batch_size=cfg.optim_args.batch_size,
        ...
        if i % cfg.eval_delta == 0:
            if cfg.data_args.k > 0:
                ...
                # res.append(curr_res)
                run.log({'iter': i, 'loss/loss': loss.item(), 'loss/loss_bigram': loss_bigram, 'loss/loss_head': loss_head})
                run.log({'iter': i , 'acc/acc_tot': acc_tot, 'acc/acc_start': acc_start, 'acc/acc_end': acc_end, 'acc/acc_end_test': acc_end_test , 'acc/acc_bigram': acc_bigram})
                # run.log({'iter' : i , 'attention_score/score_acc': score_acc, 'attention_score/score_start_acc': score_start_acc, 'attention_score/score2_acc': score2_acc})
                run.log({'iter' : i , 'matrixes/wk0_acc': wk0_acc, 'matrixes/wk0_64_acc': wk0_64_acc, 'matrixes/wk1_acc': wk1_acc, 'matrixes/wo1_acc': wo1_acc, 'matrixes/ff1_loss': ff1_loss})
